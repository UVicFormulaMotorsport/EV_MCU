...
HAL_StatusTypeDef CANsend(CAN_HandleTypeDef *hcan,
		CAN_TxHeaderTypeDef *pTxHeader, uint32_t data, uint32_t *pTxMailbox);
/* USER CODE END PFP */
...
int plausibility_check(float ap_percent, float bp_percent,
		uint8_t *IMPLAUSIBLE_STATE) {
	...
	if (*IMPLAUSIBLE_STATE == true) {
		if (ap_percent < 5) {
			*IMPLAUSIBLE_STATE = false;
			StatusFlags = 0;
			transmit_result = CANsend(&hcan1, &flagsTxHeader, StatusFlags,
					&pTxMailbox);
			HAL_Delay(10);
			return true;
		} else {
		...
		if (ap_percent >= 25 && bp_percent >= 90) {
			*IMPLAUSIBLE_STATE = true;
			StatusFlags = 0xFFFF;
			transmit_result = CANsend(&hcan1, &aPedPTxHeader, StatusFlags,
					&pTxMailbox);
			return false;
		} else {
			return true;
...
void driving_loop(void *argument) {
	...
	while (1) {
		...
		transmit_result = CANsend(&hcan1, &aPedPTxHeader, ap_percent_i,
				&pTxMailbox); // A pedal percent
		transmit_result = CANsend(&hcan1, &aPedRTxHeader, ap_raw_i,
				&pTxMailbox); // A pedal raw
		transmit_result = CANsend(&hcan1, &bPedPTxHeader, bp_percent_i,
				&pTxMailbox); // B pedal percent
		transmit_result = CANsend(&hcan1, &bPedRTxHeader, bp_raw_i,
				&pTxMailbox); // B pedal raw
		...
		transmit_result = CANsend(&hcan1, &setpointTxHeader, trq_setpoint_mod,
				&pTxMailbox);

		// Convert to string and print to serial
		sprintf(msg,
				"AP_RAW - %lu, AP_PERCENT - %lu, BP_RAW - %lu, BP_PERCENT - %lu, SP - %i\r\n",
				ap_raw_i, ap_percent_i, bp_raw_i, bp_percent_i, trq_setpoint);
		transmit_result = HAL_UART_Transmit(&huart2, (uint8_t*) msg,
				strlen(msg), 1000);
	}
}

void ready_to_drive_loop(void *argument) {
	while (1) {
...
int main(void) {
	...
	/* add threads, ... */
	osThreadNew(driving_loop, NULL, NULL);
	osThreadNew(ready_to_drive_loop, NULL, NULL);
