  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1) {
		...
		if (plausibility_check(ap_percent, bp_percent, implausible_state)) {
			...
			demanded_torque_calculator_U.throttle = ap_percent;
			...
			// TODO: May need to trigger shutdown circuit, not sure.
			dmd_trq = 0;
		}
		...
		//sprintf(msg, "Input percent: %hu, Output Percent: %hu, Output Torque: %hu\r\n", input_percent, output_percent, dmd_trq); // [percent] or [dmd_trq]
